	...
	public boolean start() {
		...
		while(true) {
			...
			if (press == Button.ID_ESCAPE) {
				...
				if (selection == 1) {
					//Select p constant
					NumberSelector kpSelect = new NumberSelector(3, this.config.getPID_kp());
					this.config.setPID_kp(kpSelect.select());
				}else if (selection == 2) {
					//Select i constant
					NumberSelector kiSelect = new NumberSelector(3, this.config.getPID_ki());
					this.config.setPID_ki(kiSelect.select());
				}else if (selection == 3) {
					//Select d constant
					NumberSelector kdSelect = new NumberSelector(3, this.config.getPID_kd());
